...
class NDArray:
    ...
    def permute(self, new_axes):
        ...
        Returns:
            ...
            strides changed).
        """
        new_shape = tuple([self._shape[axe] for axe in new_axes])
    ...
    def broadcast_to(self, new_shape):
        ...
        """
        assert len(new_shape) == len(self._shape), "must broadcast to the same dim"
        new_strides = []
        for i, s in enumerate(self._shape):
            ...
            if s == 1:
                ...
                continue
            raise Exception("can't broadcast a dim where size != 1")
        return NDArray.make(
            new_shape,
    ...
    def __getitem__(self, idxs):
        ...
        new_shape = tuple(
            [len(list(range(self.shape[i]))[s]) for i, s in enumerate(idxs)]
        )
    ...
    def flip(self, axes):
        ...
        """
        axes = axes or tuple(range(self.ndim))
        new_strides = tuple(
            [-s if i in axes else s for i, s in enumerate(self.strides)]
        )
        new_offset = self._offset
        for i, s in enumerate(self.strides):
            if i not in axes:
                continue
            new_offset += (self._shape[i] - 1) * s
        view = NDArray.make(
            self._shape, new_strides, self._device, self._handle, new_offset
        )
        return view.compact()
    ...
    def pad(self, axes):
        ...
        idxs = tuple(
            [slice(a[0], None) if a[1] == 0 else slice(a[0], -a[1]) for a in axes]
        )
        out[idxs] = self
        return out
...
def add(a, scalar, dtype):
    return a + scalar


def multiply(a, scalar, dtype):
    return a * scalar


def divide(a, scalar, dtype):
    return a / scalar


def power(a, scalar, dtype):
    return a**scalar
