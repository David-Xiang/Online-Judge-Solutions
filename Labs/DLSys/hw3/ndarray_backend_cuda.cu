...
#define COMMA ,
#define ARRAY_UNARY_OP(func_name, op)                                          \
  __global__ void func_name##Kernel(const scalar_t *a, scalar_t *out,          \
                                    size_t size) {                             \
...
#define ARRAY_BINARY_SCALAR_OP(func_name, op_pre, op_mid)                      \
  __global__ void func_name##Kernel(const scalar_t *a, scalar_t val,           \
                                    scalar_t *out, size_t size) {              \
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;                        \
    if (gid >= size)                                                           \
      return;                                                                  \
    out[gid] = op_pre(a[gid] op_mid val);                                      \
  }                                                                            \
  void func_name(const CudaArray &a, scalar_t val, CudaArray *out) {           \
    CudaDims dim = CudaOneDim(out->size);                                      \
    func_name##Kernel<<<dim.grid, dim.block>>>(a.ptr, val, out->ptr,           \
                                               out->size);                     \
  }

#define ARRAY_BINARY_EWISE_OP(func_name, op_pre, op_mid)                       \
  __global__ void func_name##Kernel(const scalar_t *a, const scalar_t *b,      \
                                    scalar_t *out, size_t size) {              \
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;                        \
    if (gid >= size)                                                           \
      return;                                                                  \
    out[gid] = op_pre(a[gid] op_mid b[gid]);                                   \
  }                                                                            \
  void func_name(const CudaArray &a, const CudaArray &b, CudaArray *out) {     \
    CudaDims dim = CudaOneDim(out->size);                                      \
    func_name##Kernel<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr,         \
                                               out->size);                     \
  }
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid >= size)
    return;
  size_t idx = gid;
  size_t origin_idx = offset;
  for (int i = dim - 1; i >= 0; --i) {
    origin_idx += (idx % shape.data[i]) * (strides.data[i]);
    idx = idx / shape.data[i];
  }
  out[gid] = a[origin_idx];
}
...
__global__ void EwiseSetItemKernel(const scalar_t *a, scalar_t *out,
  ...
  for (int i = dim - 1; i >= 0; --i) {
    target_idx += (idx % shape.data[i]) * (strides.data[i]);
    idx = idx / shape.data[i];
  }
  out[target_idx] = a[gid];
}
...
__global__ void ScalarSetItemKernel(size_t size, scalar_t val, scalar_t *out,
  ...
  size_t target_idx = offset;
  for (int i = dim - 1; i >= 0; --i) {
    target_idx += (gid % shape.data[i]) * (strides.data[i]);
 ...
 * signatures above.
 */
ARRAY_BINARY_EWISE_OP(EwiseMul, , *)
ARRAY_BINARY_SCALAR_OP(ScalarMul, , *)
ARRAY_BINARY_EWISE_OP(EwiseDiv, , /)
ARRAY_BINARY_SCALAR_OP(ScalarDiv, , /)
ARRAY_BINARY_SCALAR_OP(ScalarPower, powf, COMMA)
ARRAY_BINARY_EWISE_OP(EwiseMaximum, fmaxf, COMMA)
ARRAY_BINARY_SCALAR_OP(ScalarMaximum, fmaxf, COMMA)
ARRAY_BINARY_EWISE_OP(EwiseEq, , ==)
ARRAY_BINARY_SCALAR_OP(ScalarEq, , ==)
ARRAY_BINARY_EWISE_OP(EwiseGe, , >=)
ARRAY_BINARY_SCALAR_OP(ScalarGe, , >=)
ARRAY_UNARY_OP(EwiseLog, logf)
ARRAY_UNARY_OP(EwiseExp, expf)
ARRAY_UNARY_OP(EwiseTanh, tanhf)
