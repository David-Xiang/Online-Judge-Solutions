...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for n in reverse_topo_order:
        output_grads = node_to_output_grads_list[n]
        output_grad_sum = None
        for o in output_grads:
            if output_grad_sum is None:
                output_grad_sum = o
            else:
                output_grad_sum = output_grad_sum + o
        n.grad = output_grad_sum
        if not n.op: # no input
            continue
        input_grads = n.op.gradient(output_grad_sum, n)
        if not isinstance(input_grads, tuple): # single input
            input_grads = (input_grads,)
        for idx, i in enumerate(n.inputs):
            if i not in node_to_output_grads_list:
                node_to_output_grads_list[i] = []
            node_to_output_grads_list[i].append(input_grads[idx])
