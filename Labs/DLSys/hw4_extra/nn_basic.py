...
class Linear(Module):
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        if n_dim > 2:
            combine_dims: Tuple[int] = X.shape[:-1]
            c = math.prod(combine_dims)
            res = X.reshape((c, X.shape[-1]))
        else:
            res = X
        ...
        if n_dim > 2:
            res = res.reshape(combine_dims + (res.shape[-1],))
        return res
...
class LayerNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        broadcast_shape = (1,) * (n_dim - 1) + (self.dim,)

        x_mean = ops.summation(x, axes=sum_axis) / self.dim
        x_mean_b = ops.broadcast_to(ops.reshape(x_mean, reduce_shape), x.shape)
        x_var = ops.summation((x - x_mean_b) ** 2, axes=sum_axis) / self.dim
        x_std = (x_var + self.eps) ** (1 / 2)
        x_std_b = ops.broadcast_to(ops.reshape(x_std, reduce_shape), x.shape)

        weight_2d = ops.broadcast_to(ops.reshape(self.weight, broadcast_shape), x.shape)
        bias_2d = ops.broadcast_to(ops.reshape(self.bias, broadcast_shape), x.shape)
        return weight_2d * ((x - x_mean_b) / x_std_b) + bias_2d
